...
dcmt <- readRDS("dcmt.rds")


# make sample date set
set.seed(1222)
dcmt_samp <- dcmt %>% group_by(category) %>% sample_frac(size = 0.1, replace = FALSE)
...
dcmt_samp_trgm_cnt <- dcmt_samp_trgm %>% count(trgram, sort = TRUE)


dcmt_samp_trgm_cnt %>% filter(str_detect(trgram, "^case of"))